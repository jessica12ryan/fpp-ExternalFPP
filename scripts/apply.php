...
/**
 * Writes every Admin user to the Apache group file (mod_authz_groupfile).
...
function efppBuildVhostBody($backendPort, $htpasswdFile, $loginPageFile, $changePwFile, $https, $groupsFile, $accessDeniedFile) {
    ...
    $lines[] = '    # login (unauthenticated requests go to the login page, where the form';
    $lines[] = '    # POSTs back and Apache sets the session cookie on success). This closes';
    $lines[] = '    # the trailing-slash/path-info bypass where /settings.php/ matched neither';
    $lines[] = '    # the old negative-lookahead nor the anchored admin block.';
    $lines[] = '    # ORDERING IS LOAD-BEARING: Location/LocationMatch sections apply in file';
    $lines[] = '    # order and (AuthMerging Off default) a later section with its own';
    $lines[] = '    # authorization REPLACES earlier ones. This base rule MUST come first: the';
    $lines[] = '    # public pages and admin-only blocks below override it. If it is moved';
    $lines[] = '    # after them, /login.html redirects to itself (loop) and the admin blocks';
    $lines[] = '    # stop enforcing the group.';
    $lines[] = '    <Location />';
    $lines[] = '        AuthType Form';
    ...
    $lines[] = '    # /+ covers //settings.php. This block appears after <Location /> so it';
    $lines[] = '    # replaces the base rule here: visitors without a session still go to the';
    $lines[] = '    # login page, while logged-in non-Admin accounts see the access-denied';
    $lines[] = '    # page via the ErrorDocument below. Membership is read from the group file';
    $lines[] = '    # on every request, so role changes apply without reload.';
