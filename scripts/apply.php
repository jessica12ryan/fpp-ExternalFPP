...
function efppBuildApacheConf($port, $httpsPort, $backendPort, $htpasswdFile, $loginPageFile, $changePwFile, $enforceHttps) {
    $port = (int)$port;
    ...
    // HTTP vhost: a password-protected reverse proxy (or a redirect to HTTPS
    // when "Enforce https" is turned on).
    ...
    if ($enforceHttps && $httpsPort > 0 && $httpsPort !== $port) {
        // "Enforce https" is on: forward every request (login page included) to
        // the HTTPS listener, preserving the host and path. %1 is the host part
        // of the Host header (stripped of its port).
        $lines[] = '    # "Enforce https" is on: forward everything to the HTTPS port.';
        $lines[] = '    RewriteEngine On';
        $lines[] = '    RewriteCond %{HTTPS} !=on';
        $lines[] = '    RewriteCond %{HTTP_HOST} ^([^:]+)';
        $lines[] = '    RewriteRule ^(.*)$ https://%1:' . $httpsPort . '$1 [R=301,L]';
        $lines[] = '';
        $lines[] = '    # Never an open proxy, even on the plain-HTTP listener.';
        $lines[] = '    ProxyRequests Off';
    } else {
...
function efppApply() {
    ...
    if ($enforceHttps && !file_exists(SSL_CERT_FILE)) {
        $errors[] = 'Enforce https is enabled but no TLS certificate was found at ' . SSL_CERT_FILE . '.';
    }
    ...
    if ($enabled) {
        $conf = efppBuildApacheConf($port, $httpsPort, $backendPort, HTPASSWD_FILE, LOGIN_PAGE_FILE, CHANGE_PW_FILE, $enforceHttps);
        ...
        $messages[] = 'External web access enabled on port ' . $port . ($httpsPort > 0 && $httpsPort !== $port ? ' (https ' . $httpsPort . ')' : '') . '.';
